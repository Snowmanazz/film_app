...
import { View, Text, StyleSheet, Image, ScrollView, Platform, Alert, ActivityIndicator } from 'react-native';
...
import { getUserProfile, getWatchHistory, getFavorites } from '../../app/api/endpoints';
  ...
  return (
      <View style={styles.container}>
        ...
        <GlassView
            ...
            intensity={80}
        >
          <Image source={{ uri: user?.avatar || 'https://i.pravatar.cc/150' }} style={styles.avatarSmall} />

          <View style={styles.headerInfo}>
            <Text style={styles.headerName}>{user?.nickname || 'Guest'}</Text>
            {user?.vip?.isVip && (
            <View style={styles.vipBadge}>
              <Text style={styles.vipText}>VIP {user?.vip?.expireDate}</Text>
            </View>
            )}
          </View>
        ...
        >
          ...
          {history.map((item) => (
              <GlassView key={item.id} style={styles.historyItem} intensity={50}>
                ...
                <View style={styles.hInfo}>
                  ...
                  <Text style={styles.continueText}>上次看到 {item.time}</Text>
                </View>
          ...
          <SectionHeader title="本地缓存" icon={Download} rightText="管理" />
           {/* Mock Downloads since API doesn't have it yet */}
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.hScroll} contentContainerStyle={styles.hScrollContent}>
            {[1,2].map(i => (
                <BouncyBtn key={i} style={styles.vCard}>
                  <Image source={{ uri: `https://picsum.photos/id/${30+i}/200/300` }} style={styles.vPoster} />
                  <View style={styles.downloadBadge}>
                    <Text style={styles.downloadSize}>1.2GB</Text>
                  </View>
                  <Text style={styles.vTitle} numberOfLines={1}>示例缓存 {i}</Text>
                </BouncyBtn>
            ))}
          </ScrollView>
